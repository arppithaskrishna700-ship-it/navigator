...
async function fetchPlaces() {
  ...
  try {
    ...
    showLoading(false);
    document.getElementById('locationStatus').textContent = '❌ Error fetching places.';
  }
}
...
function renderCards(places) {
  ...
  if (places.length === 0) {
    grid.innerHTML = `<div class="no-results"><span>🔍</span>No spots found with this filter.</div>`;
    return;
  }
  ...
  places.forEach((place, i) => {
    ...
    card.innerHTML = `
      ...
      <div class="card-body">
        ...
        <button class="nav-btn" onclick="event.stopPropagation(); navigateTo(${place.lat}, ${place.lng}, '${place.name.replace(/'/g, "\\'")}')">
          🧭 Navigate
        </button>
      </div>`;
...
// Option 1: In-app route on the map
    ...
    createMarker: function(i, wp) {
      return L.marker(wp.latLng, {
        icon: L.divIcon({
          className: '',
          html: i === 0
            ? `<div style="background:#ff6b35;width:20px;height:20px;border-radius:50%;border:3px solid white;"></div>`
            : `<div style="background:#06d6a0;width:24px;height:24px;border-radius:50%;border:3px solid white;display:flex;align-items:center;justify-content:center;font-size:12px;">🍽</div>`,
          iconSize: [24, 24],
          iconAnchor: [12, 12]
        })
      });
    },
    show: false, // hides the turn-by-turn panel (cleaner look)
  }).addTo(map);
...
// Option 2: Open Google Maps on phone (best for actual phone GPS)
function openGoogleMaps(lat, lng, name) {
  const url = `https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}&destination_place_name=${encodeURIComponent(name)}&travelmode=walking`;
  window.open(url, '_blank');
}

// Option 3: Open in Apple Maps (for iPhone users)
  ...
  places.forEach(place => {
    ...
    marker.bindPopup(`
      <div style="font-family:'DM Sans',sans-serif;min-width:160px;padding:4px">
        ...
        <span style="color:#888;font-size:12px">${place.type}</span><br><br>
        <button onclick="navigateTo(${place.lat}, ${place.lng}, '${place.name.replace(/'/g, "\\'")}')"
          style="background:#ff6b35;color:white;border:none;padding:6px 14px;border-radius:20px;cursor:pointer;font-size:12px;margin-right:6px">
          🧭 Route
        </button>
        <button onclick="openGoogleMaps(${place.lat}, ${place.lng}, '${place.name.replace(/'/g, "\\'")}')"
          style="background:#4285f4;color:white;border:none;padding:6px 14px;border-radius:20px;cursor:pointer;font-size:12px">
          📱 Google Maps
        </button>
      </div>
    `);
...
function openModal(place) {
  ...
  document.getElementById('modalContent').innerHTML = `
    ...
    ${place.phone ? `<div class="modal-phone" style="margin-bottom:12px">📞 ${place.phone}</div>` : ''}
    ${place.openingHours ? `<div style="color:#06d6a0;font-size:0.9rem;margin-bottom:16px">🕐 ${place.openingHours}</div>` : ''}
    <div style="display:flex;gap:10px;flex-wrap:wrap">
      <button onclick="closeModal(); navigateTo(${place.lat}, ${place.lng}, '${place.name.replace(/'/g, "\\'")}')"
        class="modal-gmaps-btn">🧭 Route on Map</button>
      <button onclick="openGoogleMaps(${place.lat}, ${place.lng}, '${place.name.replace(/'/g, "\\'")}')"
        class="modal-gmaps-btn" style="background:#4285f4">📱 Google Maps</button>
      ${place.website ? `<a href="${place.website}" target="_blank" class="modal-gmaps-btn" style="background:#333">🌐 Website</a>` : ''}
    </div>
  `;
...
function showToast(msg) {
  ...
  if (!toast) {
    ...
    toast.style.cssText = `
      ...
      background:#ff6b35; color:white; padding:12px 24px; border-radius:50px;
